...
data Vehicle = Bike | Bus | Train | Tram
...
study (NthYear x) = if x > 6 then Graduated else NthYear (x + 1)
...
data UpDown = MkUp Int | MkDown Int 
...
tick :: UpDown -> UpDown
tick (MkUp x) = MkUp (x + 1)
tick (MkDown x) = MkDown (x - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (MkUp x) = MkDown x
toggle (MkDown x) = MkUp x
...
rgb (Mix c1 c2) = map (\(x, y) -> (x + y)/2) $ zip (rgb c1) (rgb c2)
...
toNat :: Int -> Maybe Nat
toNat z = if z < 0 then Nothing else Just (go z)
 where go 0 = Zero
       go x = PlusOne (go (x - 1))
...
fromBin :: Bin -> Int
fromBin b = go 0 1 b
  where go acc _ End = acc
        go acc pow (O b) = go acc (pow * 2) b
        go acc pow (I b) = go (acc + pow) (pow * 2) b        

toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin x = if even x then O (toBin bs) else I (toBin bs)
  where bs = x `div` 2
