...
firstHalf s = take fh s
    where ls = length s
          half = div ls 2
          fh = if odd ls then half + 1 else half
...
capitalize s = unwords (map capitalize' (words s)) 

capitalize' :: String -> String
capitalize' s = (toUpper (head s)) : (drop 1 s)
...
powers :: Int -> Int -> [Int]
powers k max = 1 : (takeWhile (\x -> x <= max) (powers' k))

powers' :: Int -> [Int]
powers' k = map (\x -> k^x) [1..]
...
joinToLength n s = [ fst ++ snd | fst <- s, snd <- s, let ns = fst ++ snd, (length ns == n)]
...
sumRights l = sum (map sumRights' l)
...
{-
const const "too" 12 "help"
===> (const const "too") 12 "help"
===> (const 12 "hlpe")
===> 12
-}
multiApp :: ([b] -> t2) -> [ (t1 -> b) ] -> t1 -> t2
multiApp f gs x = f (map (\f -> f x) gs)
...
interpreter cmds = interpreter' cmds 0 0

interpreter' :: [String] -> Int -> Int -> [String]
interpreter' [] _ _ = []
interpreter' (c:cs) x y = case c of
   "up"     -> interpreter' cs x (y + 1)
