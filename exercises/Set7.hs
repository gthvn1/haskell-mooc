...
add x s = go [] x s
  where go :: (Eq a, Ord a) => [a] -> a -> Set a -> Set a
        go l x (Set []) = Set (l ++ [x])
        go l x (Set (y:ys))
          | x == y = Set (l ++ (y:ys))
          | x < y  = Set (l ++ [x] ++ (y:ys))
          | otherwise = go (l ++ [y]) x (Set ys)
...
data State = Start | EggsOk | FlourOk | SugarOk | MixReady | MixOk | Finished | Error
  deriving (Eq,Show)
...
step Start AddEggs = EggsOk

step EggsOk AddFlour = FlourOk
step EggsOk AddSugar = SugarOk

step FlourOk AddSugar = MixReady
step SugarOk AddFlour = MixReady

step MixReady Mix = MixOk

step MixOk Bake = Finished
...
toList :: Num a => NonEmpty a -> [a]
toList (x :| []) = [x]
toList (x :| (y:ys)) = x : (toList $ y :| ys)

average :: Fractional a => NonEmpty a -> a
average l = (sum ll) / (fromIntegral (length ll))
  where ll = toList l
...
reverseNonEmpty l = go [] l
  where
    go :: [a] -> NonEmpty a -> NonEmpty a
    go acc (x :| []) = x :| acc
    go acc (x :| (y:ys)) = go (x : acc) (y :| ys)
------------------------------------------------------------------------------
...
instance Ord a => Semigroup (Set a) where
  Set s1 <> Set s2 = go [] s1 s2
    where
      go acc s1 [] = Set (acc ++ s1)
      go acc [] s2 = Set (acc ++ s2)
      go acc (x:xs) (y:ys)
        | x == y    = go (acc ++ [x]) xs ys
        | x < y     = go (acc ++ [x]) xs (y:ys)
        | otherwise = go (acc ++ [y]) (x:xs) ys
...
passwordAllowed p (ContainsSome (c:cs)) = if elem c p
                                          then True
                                          else passwordAllowed p (ContainsSome cs)
...
data Arithmetic = Arithmetic String Integer
  deriving Show

literal :: Integer -> Arithmetic
literal x = Arithmetic (show x) x

operation :: String -> Arithmetic -> Arithmetic -> Arithmetic
operation "+" (Arithmetic s1 x1) (Arithmetic s2 x2) =
  Arithmetic ("(" ++ s1 ++ "+" ++ s2 ++ ")") (x1 + x2)
operation "*" (Arithmetic s1 x1) (Arithmetic s2 x2) =
  Arithmetic ("(" ++ s1 ++ "*" ++ s2 ++ ")") (x1 * x2)

evaluate :: Arithmetic -> Integer
evaluate (Arithmetic _ x) = x

render :: Arithmetic -> String
render (Arithmetic s _) = s
